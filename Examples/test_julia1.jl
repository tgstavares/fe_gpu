...
tt2 = time()
result = reg(df, @formula(wage ~ tenure + sick_shock + extra_1 + extra_2 + extra_3 + extra_4 + extra_5 + extra_6 + fe(worker) + fe(firm) + fe(time)), Vcov.cluster(:worker,:firm,:time), method=:CUDA, tol=1e-8)
println("")
